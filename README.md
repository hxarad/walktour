...
| _maskFillColor_ | string | Fill color of the mask overlay. Default is `black`. |
| _maskOpacity_ | number | Opacity of the mask overlay. Default is `0.3`. |
| _maskBackdropFilter_ | string | CSS `backdrop-filter` applied to the masked area (not the cutout). |
